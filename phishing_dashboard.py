...
```
    ...
    for legit in LEGIT_DOMAINS:
        ...
        if base in domain and legit not in domain:
            signals.append(f"🎭 Possible lookalike of {legit}")
            score += 0.35

    # Hyphens
    hyphen_count = domain.count("-")
    if hyphen_count >= 2:
        signals.append(f"⚠️ {hyphen_count} hyphens in domain (suspicious)")
        score += 0.15

    # Keyword in URL
    kw_hits = [kw for kw in CRITICAL_KEYWORDS if kw in domain + path]
    if kw_hits:
        signals.append(f"⚠️ Sensitive keywords in URL: {', '.join(kw_hits)}")
        score += 0.20

    # URL length
    if len(url) > 100:
        signals.append(f"⚠️ Very long URL ({len(url)} chars)")
        score += 0.10

    # HTTPS check
    if parsed.scheme == "http":
        signals.append("⚠️ No HTTPS — unencrypted connection")
        score += 0.10

    # Subdomain depth
    subdomain_parts = domain.split(".")
    if len(subdomain_parts) > 4:
        signals.append(f"⚠️ Deep subdomain structure ({len(subdomain_parts)} parts)")
        score += 0.15
...
dmarc=fail
From: “PayPal Security” [noreply@paypal.com](mailto:noreply@paypal.com)
